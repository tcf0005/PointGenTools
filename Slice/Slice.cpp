...
int main(int argc, char const *argv[]) {
    ...
    while( true ) {
        ...
        //Determining the Contours
            ...
            cv::Mat contourImage(destination.size(), CV_8UC3, cv::Scalar(0,0,0));
        //Determining Contour with Max Area
            ...
            cv::drawContours(contourImage, contours, maxElement, cv::Scalar(0,255,0), CV_FILLED);
            imshow("Contour",contourImage);
         //Defining the Region of Interest
            cv::Mat ROI(contourImage, rect);
            imshow("ROI",ROI);
        //Determining the x values from "Slice" in different directions
        
            //Left Slice
            int XL, XR;
            bool Detected = false;
            for (int x=0; x<ROI.cols; x++){
                if (ROI.at<cv::Vec3b>(Recth/2,x)[1]==255){
                    XL=x;
        ...
        //If Statement that prevents Segfault if there are is no line detected in REGION of Interest
            if(Detected == true){
                ...
                //Outputting the Mean Pixel Value X Position in ROI frame
                    std::cout<<"Center: "<<Centx<<std::endl;
                ...
                //Drawing Things 
                    ...
                    //Circle of Point on Line (Detected Point)
                        cv::circle(Drawing,Point(Rectx+Centx,Recty+Recth/2),1,cv::Scalar(  89, 100, 255),2,4,0 );
